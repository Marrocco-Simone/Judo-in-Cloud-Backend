import {
  ...
  getNextMatchIdx,
  getPrevMatchIdx,
  getRecoveryBracketIdentifier,
  getRecoveryBracketIdx,
  getRecoveryBracketLocation,
  getRoundsCount
} from './bracket_utils';
...
test('builds a bracket with 3 players', () => {
  const bracket = generateMainBracket(['A', 'B', 'C'] as any[]);
...
test('builds a bracket with 5 players', () => {
  const bracket = generateMainBracket(['A', 'B', 'C', 'D', 'E'] as any[]);
...
test('throws error on invalid match selection', () => {
  const bracket = generateMainBracket(['A', 'B', 'C', 'D', 'E', 'F'] as any[]);
