...
tournament_router.get('/:tournament_id/next', async (req, res) => {
  try {
    ...
    const { winners_bracket, finished, recovered_bracket_1, recovered_bracket_2 } = tournament;

    if (finished) success(res, 'The tournament\'s finished, there are no matches left to compete');

    const winners_bracket_next = await getNextMatches({ bracket: winners_bracket });
    const recovered_bracket_next_1 = await getNextMatches({ bracket: recovered_bracket_1 });
    const recovered_bracket_next_2 = await getNextMatches({ bracket: recovered_bracket_2 });
...
async function getNextMatches ({ bracket }) {
  ...
  for (const round of bracket) {
    ...
    for (const match_id of round) {
      ...
      if (!match) throw new Error('Found match_id which doesn\'t reference an existing match');
